...
**Write paths.** The server has exactly TWO trading-relevant ones, both explicit human actions calling the Phase 3 functions verbatim: `POST /api/promote {strategyName, typedConfirmation}` → `promote()` (the real gate — score ≥ 75, exact typed name; the thrown gate reason is returned verbatim as `{ok:false, error}` with HTTP 422) and `POST /api/kill-switch {engaged, reason}` → `setKillSwitch()`. Everything else is read-only; `dashboard/server` contains no INSERT/UPDATE/DELETE statements at all (the Strategy Lab below logs its one advisory row through the Phase 4 `logRecommendation`).

**Strategy Lab.** `POST /api/strategy/lab {description|pine, fixture?: "2017"|"synthetic"}` (and the full-width Lab panel in the UI) wires the Phase 4 translator to the Phase 1 backtester, fully offline: plain English or Pine → the real `translateStrategyText` core (stub provider when no `AI_API_KEY`) → `assertValidStrategy` → `backtest()` over a committed fixture (default the real 8,760-candle 2017 Bitfinex year) with a buy&hold comparison, returned as `{strategy, metrics, buyHoldPct, equityCurve}` plus one `recommendations` row (`type: operation`). Schema-invalid model output returns HTTP 422 with the real validation errors. Advisory only: the generated strategy is NOT written to `/strategies`, nothing touches `orders`/`fills`/`positions`/`portfolios`/`live_orders`, and no promote/live path is reachable — a Lab strategy still has to paper-trade, clear confidence ≥ 75, be promoted with a typed confirmation, and pass the Phase 6 gauntlet before any live order.
